...
#
# The stack is deployed in two passes. CloudFormation cannot upload local source
# itself, and the durable worker function needs its real code before a version is
# published, so this script builds the deployment package first, stages it in S3, and
# then points the stack at it.
#
# Usage:
#   ./deploy.sh --farm-id farm-xxxx [--stack-name NAME] [--region REGION]
#               [--model-id MODEL] [--max-workers N]
...
echo "==> Confirming ${MODEL_ID} is available in ${REGION}"
if ! aws bedrock list-foundation-models --region "$REGION" \
      --query "modelSummaries[?modelId=='${MODEL_ID}'].modelId" --output text | grep -q .; then
  echo "error: model ${MODEL_ID} is not available in ${REGION}." >&2
  echo "       Asynchronous invocation requires a model that supports StartAsyncInvoke." >&2
...
# Bundle the durable execution SDK rather than relying on the copy in the runtime, so
# a runtime update cannot change the behavior of in-flight executions.
...
# A content-addressed key makes each deploy a distinct S3 object, which is what
# causes CloudFormation to publish a new function version.
